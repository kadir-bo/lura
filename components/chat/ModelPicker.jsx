    ...
    <div className="flex flex-col" style={{ width: 280, maxHeight: 360 }}>
      ...
      >
        ...
        <button
          ...
          className="flex items-center justify-center w-8 h-8 rounded-lg shrink-0 transition-colors duration-100 hover:bg-overlay text-text-secondary"
          title="Manage models"
        ...
        {grouped.length === 0 ? (
          ...
          grouped.map(([provider, list]) => (
            <div key={provider} className="flex flex-col">
              <p
                className="px-2.5 pt-2 pb-1 text-[11px] font-medium uppercase tracking-wider text-text-muted"
              >
                {provider}
              </p>
              {list.map((m) => {
